...
// Read form-data or JSON body (clients may POST JSON)
...
if (is_array($jsonBody)) {
    if (empty($postImage) && isset($jsonBody['postImage'])) $postImage = $jsonBody['postImage'];
    if (empty($postImageUrl) && isset($jsonBody['postImageUrl'])) $postImageUrl = $jsonBody['postImageUrl'];
    if (empty($caption) && isset($jsonBody['caption'])) $caption = $jsonBody['caption'];
    if (isset($jsonBody['timestamp'])) $timestamp = intval($jsonBody['timestamp']);
}

$timestamp = intval($timestamp ?? ($_POST['timestamp'] ?? (time() * 1000)));
