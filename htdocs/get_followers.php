...
$stmt2 = $conn->prepare("SELECT f.follower_id, u.username, u.imageBase64 FROM followers f JOIN users u ON f.follower_id = u.id WHERE f.following_id = ? ORDER BY f.created_at DESC");
...
while ($row = $res->fetch_assoc()) {
    $followers[] = [
        "id" => (int)$row['follower_id'],
        "username" => $row['username'],
        "imageBase64" => $row['imageBase64']
    ];
}

json_response(["success"=>true, "followers"=>$followers]);
?>